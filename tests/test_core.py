...
def test_dataset_protocols_are_explicit_and_distinct():
    ce = CrackEnsemblesProtocol()
    cs = CrackStructuresProtocol()
    assert ce.threshold == 0.60
    assert ce.eps_ratio == 0.01
    assert cs.threshold == 0.25
    assert cs.eps_m == 0.035
    assert cs.max_gap_m == 0.04
    assert cs.min_samples_cluster == 1
