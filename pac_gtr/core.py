...
class Protocol:
    """Frozen, scale-normalized PAC-GTR parameters."""
...
def predict_components(
    ...
    *,
    ...
    labels = graph_dbscan(
        ...
        protocol.eps_ratio * diagonal,
        protocol.min_samples,
        adaptive=adaptive,
    ...
    for label in np.unique(labels[labels >= 0]):
        ...
        if len(local) >= protocol.min_component_points:
    ...
    if mode in {"gtr", "full"} and len(components) > 1:
        groups = _merge_groups(
            components,
            protocol.topology_gap_ratio * diagonal,
            protocol.merge_alignment_cos,
        )
        components = [
